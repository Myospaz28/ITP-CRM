...
export const addUser = async (user) => {
  const query =
    'INSERT INTO users (name, contact_no, email, address, role, status, username, password) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
...
export const getUsers = async () => {
  const query =
    'SELECT user_id, name, contact_no, email, address, role, status FROM users ORDER BY user_id DESC';
...
export const editUser = async (user_id, user) => {
  let query = `
    UPDATE users 
    SET name = ?, contact_no = ?, address = ?, email = ?, role = ?, status = ?
  `;
  ...
  // 🔐 sirf tab password update karo jab diya ho
  if (user.password) {
    query += `, password = ?`;
    values.push(user.password); // already hashed
  }
