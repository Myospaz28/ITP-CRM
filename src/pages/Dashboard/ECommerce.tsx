...
//   const [invalidLeads, setInvalidLeads] = useState<number | null>(null);
...
//       try {
//         setIsLoading(true);
//         const response = await fetch(`${BASE_URL}api/dashboard/users/count`);
//         const rawData = await response.json();
//         setTotalLeads(rawData.total_users || 0);
...
//         setfollowups(partsData.active_count || 0);
...
//         setMeetingScheduled(meetingData.win_count || 0);
...
//           `${BASE_URL}api/dashboard/master-data/loose-leads`,
//           {
//             credentials: 'include',
//           },
//         );
//         const pendingData = await pendingRes.json();
//         setpendingLeads(pendingData.lose_count || 0);

//         const invalidReg = await fetch(
//           `${BASE_URL}api/dashboard/master-data/invalid-leads`,
//           {
//             credentials: 'include',
//           },
//         );
//         const invalidData = await invalidReg.json();
//         setInvalidLeads(invalidData.invalid_count || 0);
...
//   // ⭐ Navigation Handlers
//   const handleGoToTotalLeads = () => navigate('/user/user-list');
...
//   const handleGoToPendingLeads = () => navigate('/loseleads');
//   const handleGoToInvalidLeads = () => navigate('/invalidleads');
...
//       borderColor: 'border-red-200/50',
//       // animation: { rotate: [0, 180, 0], scale: [1, 1.02, 1] },
//       onClick: handleGoToInvalidLeads,
//     },
//     {
//       title: 'Category',
//       value: totalcategory,
...
const ECommerce: React.FC = () => {
  ...
  const [assignedTeleCallerCount, setAssignedTeleCallerCount] = useState(0);
  const [assignedTeleCallerNames, setAssignedTeleCallerNames] = useState('');
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // Only fetch category and product data if user is admin
        if (userRole === 'admin') {
          const categoryRes = await fetch(
            `${BASE_URL}api/dashboard/master-data/category`,
          );
        ...
        // ✅ Team Lead → Assigned Telecaller Count

        const teleCallerRes = await fetch(
          `${BASE_URL}api/dashboard/team-lead/count`,
          { credentials: 'include' },
        );
        console.log('teleCallerRes', teleCallerRes);

        const teleCallerData = await teleCallerRes.json();

        setTeamLeads(teleCallerData.tele_caller_count || 0);
        setAssignedTeleCallerNames(teleCallerData.tele_caller_names || '');

        // ✅ New Leads Count
        const newLeadsRes = await fetch(
          `${BASE_URL}api/dashboard/new-leads-count`,
          { credentials: 'include' },
        );
        const newLeadsData = await newLeadsRes.json();
        setNewLeads(newLeadsData.new_lead_count || 0);
      } catch (err) {
        console.error(err);
      } finally {
        setIsLoading(false);
    ...
    // Only fetch dashboard data if userRole is determined
    if (userRole !== null) {
      fetchDashboardData();
    }
  }, [userRole]);
  ...
  const handleGoToTotalLeads = () => navigate('/user/user-list');
  const handleGoToAssignedLeads = () => navigate('/call');
  const handleGoToFollowups = () => navigate('/activeleads');
  const handleGoToMeetingScheduled = () => navigate('/winleads');
  const handleGoToCategory = () => navigate('/master/category');
  const handleGoToProducts = () => navigate('/master/product');
  const handleGoToConvertedLeads = () => navigate('/followup/view-campaign');
  const handleGoToPendingLeads = () => navigate('/loseleads');
  const handleGoToInvalidLeads = () => navigate('/invalidleads');
  const handleGoToAssignedTelecaller = () => navigate('/team-lead-telecallers');
  const handleGoToNewLeads = () => navigate('/newlead');
  // Base card data configuration for all users
  const baseCardData = [
    {
      title: 'Assigned Leads',
      value: assignedLeads,
...

    {
      ...
      bgGradient: 'from-indigo-500/10 to-indigo-600/5',
      borderColor: 'border-indigo-200/50',
      onClick: handleGoToAssignedTelecaller,
    },
  ];
  ...
  const getCardData = () => {
    ...
    // Base cards visible to everyone except role-specific ones
    const filteredBaseCards = baseCardData.filter((card) => {
      // Hide "Assigned Telecaller" for non-team lead users
      if (card.title === 'Assigned Telecaller' && userRole !== 'team lead') {
        return false;
      }
      return true;
    });
    ...
    if (userRole === 'admin') {
      return [...adminCardData, ...filteredBaseCards];
    } else if (userRole === 'tele-caller' || userRole === 'team lead') {
  ...
  // Loading skeleton - adapt based on user role
  if (isLoading || userRole === null) {
    const skeletonCount =
      userRole === 'admin'
        ? 8
        : userRole === 'tele-caller' || userRole === 'team lead'
        ? 5
        : 8;
