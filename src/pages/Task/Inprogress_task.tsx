...
const InprogressTask: React.FC = () => {
  ...
  //   try {
  //     const response = await fetch(BASE_URL + "api/tasks/inProgress");
  ...
  const fetchTasks = async () => {
    try {
      const response = await axios.get(`${BASE_URL}api/tasks/inProgress`, { withCredentials: true });
      const { data } = response;
  ...
  return (
    <div className="p-4">
      <Breadcrumb pageName="In Progress Part List" />
      <input
        type="text"
